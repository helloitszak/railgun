...
command :delete do |c|
	c.syntax = "mylist.rb delete [options] <files>"
...
command :purge do |c|
	c.syntax = "mylist.rb purge [options] <files>"
	c.description = "Purges files from MyList and optionally removes them from disk."
	c.option "--[no-]rm", "DANGER. Deletes the files on disk."
	c.action do |args, options|
		# Setup processor to run mylist additions
		processor = Processor.new(opts.options[:anidb], options.test)
		processor.plugins << MyListPurger.new
